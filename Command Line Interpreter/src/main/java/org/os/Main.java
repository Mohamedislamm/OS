            ...
            try {
                ...
                switch (command[0]) {
                    ...
                    case "pwd":
                        ...
                        break;
                    case "cd":
                        currentDirectory = commands.cd(command, currentDirectory);
                        break;
                    case "ls":
                    ...
                    case "zip":
                        if (command.length >= 3) {
                            commands.zip(command, currentDirectory);
                        } else {
                            System.out.println("Usage: zip <archive_name> <file1> [file2] ...");
                        }
                        break;
                    case "unzip":
                        if (command.length >= 2) {
                            commands.unzip(command, currentDirectory);
                        } else {
                            System.out.println("Usage: unzip <archive_name> [destination_directory]");
                        }
                        break;
                    default:
